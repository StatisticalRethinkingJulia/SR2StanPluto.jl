...
if !isnothing(m5_4_RSs) && !isnothing(m5_4_SRs)
  ...
  r = -2.0:0.1:3.0
  mu_SR = pmean(part5_4_SRs.a) .+ pmean(part5_4_SRs.bSR)*r

  figs[2] = plot(xlab="R (std)", ylab="S (std)", leg=false)
  plot!(r, mu_SR)
  scatter!(df[:, :S_s], df[:, :R_s])
  
  mu_RS_obs = pmean(part5_4_RSs.a) .+ pmean(part5_4_RSs.bRS)*df[:, :S_s]
  res_RS = df[:, :R_s] - mu_RS_obs

  s = -2.0:0.1:3.0
  mu_RS = pmean(part5_4_RSs.a) .+ pmean(part5_4_RSs.bRS)*s

  figs[1] = plot(xlab="S (std)", ylab="R (std)", leg=false)
  plot!(s, mu_RS)
  scatter!(df[:, :R_s], df[:, :S_s])
  
  mu_SR_obs = pmean(part5_4_SRs.a) .+ pmean(part5_4_SRs.bSR)*df[:, :R_s]
