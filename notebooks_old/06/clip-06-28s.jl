...
begin
	...
	df[!, :p] = [rand(Normal(b_GP * df[i, :g] + b_U * df[i, :u]), 1)[1] 
		for i in 1:N]
	df[!, :c] = 
		[rand(Normal(b_PC * df[i, :p] + b_GC * df[i, :g] + b_U * df[i, :u]), 1)[1] 
		for i in 1:N]
...
# ╔═╡ a5a491fa-3d70-4b42-b3f4-01153989010f
stan6_11 = "
data {
    int <lower=0> N;
    vector[N] C;
...
# ╔═╡ be268f40-32a9-4714-bc24-36789f78ae84
begin
	m6_11s = SampleModel("m6.11s", stan6_11)
	rc6_11s = stan_sample(m6_11s; data)
end;
...
begin
	...
	rc6_12s = stan_sample(m6_12s; data)
