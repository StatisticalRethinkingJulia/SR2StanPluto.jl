...
let
    ...
    annotations!("1.0", position=(-0.105, 0.205), textsize=15)
    annotations!("1.3", position=(-0.225, 0.027), textsize=15)
    annotations!("1.11=2.0", position=(-0.09, -0.285), textsize=15)
    annotations!("2.8", position=(0.0, -0.1), textsize=15)
    lines!([-0.12, -0.11], [0.175, 0.14]; color=:black, linewidth=0.6)
    annotations!("p = (-1.30, -1.68)", position=(-0.125, 0.12), textsize=12)
    lines!([0.021, 0.06], [-0.13, -0.11]; color=:black, linewidth=0.6)
    annotations!("p = (0.46, 2.13)", position=(0.062, -0.11), textsize=12)
    current_figure()
end
...
manifest_format = "2.0"
project_hash = "c978248364f9cb6dcd4586bedd6f8a80b8eb77f8"
