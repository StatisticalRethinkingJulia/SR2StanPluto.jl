...
let
    ...
    ax = Axis(f[1, 2]; title="Density at radial distance", xlabel="Radial distance from mode", ylabel="Density")
    for (ind, n) in enumerate([1, 10, 100, 1000])
        m = rand(MvNormal(zeros(n), Diagonal(ones(n))), Int(1e3))
        rd = rad_dist.([m[:, i] for i in 1:1000])
        density!(rad_dist.(rd), color=colors[ind])
        annotations!("$n", position = (positions[ind], heights[ind]), textsize=15)
    end
    annotations!("Dimensions: [1, 10, 100, 1000]", position = (3, 0.8), textsize=15)
    current_figure()
end
