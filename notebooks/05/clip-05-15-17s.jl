...
if !isnothing(m5_3s)
	begin
		...
		v = zeros(size(df, 1), 4);
		for i in 1:N
			mu = pmean(part5_3s.bM) * df[i, :Marriage_s] + 
				pmean(part5_3s.bA) * df[i, :MedianAgeMarriage_s]
			if i == 13
			...
			s = rand(Normal(mu, pmean(part5_3s.sigma)), 1000)
		...
		df2 = DataFrame(
			:x => df.Divorce_s,
			:y => [pmean(part5_3s.bM) * df[i, :Marriage_s] + 
				pmean(part5_3s.bA) * df[i, :MedianAgeMarriage_s] for i in 1:N]
		)
