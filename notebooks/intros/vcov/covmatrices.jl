...
# ╔═╡ 79401160-3711-4efb-92de-4cdfc081e2d6
using Pkg, DrWatson

# ╔═╡ d0fc71bf-efa8-4080-9e82-2b13c0964804
begin
...
# ╔═╡ 732f89a6-ee7a-4460-aeee-823105d2e1de
md"### Explorations with CovarianceMatrices.jl."

# ╔═╡ d6d979a1-f7b0-4805-8ac2-d69758ab3333
begin
	Random.seed!(1)
	n = 500
	x = randn(n, 5)
...
# ╔═╡ 70fd6c88-631d-4005-9164-abc5b08ad0e0
PRECIS(df)

# ╔═╡ ad20d2d0-b3bf-4eab-9e44-85f324d8c780
md"##### Using the data in df, the coefficient of the regression can be estimated using GLM."

# ╔═╡ 5707fa4a-f605-4c51-ab98-bb595568b56f
lm1 = glm(@formula(y~x1+x2+x3+x4+x5), df, Normal(), IdentityLink())

# ╔═╡ 713c1771-39b6-447a-ac1c-fa6c5a8a7c96
vcov1 = vcov(QuadraticSpectralKernel{Andrews}(), lm1, prewhite = false)

# ╔═╡ 2c06c1bc-1509-4a4d-9b4d-fc14b42a3cbe
md"##### For the previous example:"

# ╔═╡ cc259c16-963f-429a-b82f-f2f54d91c4f2
stderror(QuadraticSpectralKernel{Andrews}(), lm1, prewhite = false)

# ╔═╡ a0e74b7f-ca0f-48fe-b979-542e6ea3fbc2
stderror(QuadraticSpectralKernel{NeweyWest}(), lm1, prewhite = false)

# ╔═╡ 93d14329-5974-4568-861c-c7533d63fb02
md"##### Sometime is useful to access the bandwidth selected by the automatic procedures. This can be done using the optimalbandwidth method."

# ╔═╡ 31225f29-c26b-4e99-93a6-a64b416f9285
optimalbandwidth(QuadraticSpectralKernel{NeweyWest}(), lm1; prewhite = false)

# ╔═╡ f9eecf64-64b9-4848-88d5-452cf7e1c76d
optimalbandwidth(QuadraticSpectralKernel{Andrews}(), lm1; prewhite = false)

# ╔═╡ e50b1e20-0e13-477c-bdf0-f8ba63e97609
begin
...
# ╔═╡ fc576989-86e4-4fce-a0d1-2bfd1ff4bfb3
begin
	X   = [ones(n) x]
	_,K = size(X)
	b   = X\y
	...
	B   = inv(X'X)      
	bw1 = CovarianceMatrices.optimalbandwidth(kernel, momentmatrix, prewhite=false)
end

# ╔═╡ 7d11a551-545b-43b6-a4a3-d9ad53ff46d5
begin
	# df adjustment is built into vcov
	A   = lrvar(QuadraticSpectralKernel(bw1), momentmatrix, scale = n^2/(n-K))   
	Σ   = B*A*B
	Σ .- vcov(QuadraticSpectralKernel(bw1), lm1, dof_adjustment=true)
end

# ╔═╡ 61b1dd77-b049-43ae-abf2-bb69e3f223f7
X

# ╔═╡ b13f227e-a1c1-45af-8391-38b566ac8353
b

# ╔═╡ cb2cfe7a-6e83-4c44-a94f-fbed511402d5
stan_vcov = "
data {
	int N;
	int M;
...
# ╔═╡ 5932f565-7d5a-484f-921c-e97e162ea843
data = (N=n, M=size(X, 2), y=y, x=X);

# ╔═╡ 62ac5b7d-f6ca-4ed4-baad-606839e7082f
begin
	sm = SampleModel("vcov", stan_vcov)
	rc = stan_sample(sm; data)
end

# ╔═╡ fa8401d0-1e96-4bac-be41-c837b86d91a5
if success(rc)
	dfa = read_samples(sm, :dataframe)
	PRECIS(dfa)
end

# ╔═╡ ba0bfcdf-bc2a-46f1-a1a2-93f385d8f7b5
lm1

# ╔═╡ 6fd98d83-f321-4d51-a03b-a039b63870fd
X1 = dfa[:, 1:6]

# ╔═╡ 337e7a3e-3669-43d4-ae42-496b6dceb142
b

# ╔═╡ b5e07484-6756-4518-a6e4-d617b33460e5
mean(Array(X1); dims=1)

# ╔═╡ adfab6ce-df37-4e1e-beac-21e3b2e0be96
let
	res = y .- X*mean(Array(X1); dims=1)'
	momentmatrix = X.*res
	# Jacobian of moment conditions
	B   = inv(X'X)      
	bw1 = CovarianceMatrices.optimalbandwidth(kernel, momentmatrix, prewhite=false)
...
# ╔═╡ a3213bf9-09bb-412e-bca9-cdd9d4f4c10c
vcov1

# ╔═╡ 0fc926ce-abf1-4576-baa7-ffbf709e3d09
Σ1 .- vcov1
...
[compat]
CovarianceMatrices = "~0.10.3"
DataFrames = "~1.3.0"
DrWatson = "~2.7.5"
GLM = "~1.5.1"
StanSample = "~5.2.1"
StatisticalRethinking = "~4.4.5"
"""
...
git-tree-sha1 = "116d3005fcd778f6c2d9af72711b206f9dcaa89e"
...
git-tree-sha1 = "616651cb7e8811ca469efddfe5cf950952524fec"
...
git-tree-sha1 = "4f7c5dac8982711d71e9b67794f934057961ea65"
...
git-tree-sha1 = "1bd072c38ba372b219a5b77f434ae5b304234ad0"
...
git-tree-sha1 = "027c09d8da64ac611b96da09f69410eb23ed4cd4"
...
git-tree-sha1 = "3085368ec9d789c31f6eda9e0cd68d5d77073d17"
