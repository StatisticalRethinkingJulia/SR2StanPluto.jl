...
if success(rc5_1s)
    pt5_1s = read_samples(m5_1s, :particles)
    NamedTupleTools.select(pt5_1s, (:a, :bA, :sigma))
end

# ╔═╡ f08ff6a1-ed68-4441-984a-55baa3eef109
if success(rc5_2s)
    pt5_2s = read_samples(m5_2s, :particles)
    NamedTupleTools.select(pt5_2s, (:a, :bM, :sigma))
end

# ╔═╡ 724af235-a95d-4951-a1a1-a9f61af83964
if success(rc5_3s)
    pt5_3s = read_samples(m5_3s, :particles)
    NamedTupleTools.select(pt5_3s, (:a, :bA, :bM, :sigma))
end

# ╔═╡ 4f78201d-af67-42ef-8c97-d0686f491cb7
md"
!!! note

Unfortunately the output goes to the terminal."
...
# ╔═╡ 47678e39-a5c8-479a-aa97-13f15382dc48
begin
	nt5_1s = read_samples(m5_1s, :namedtuple)
    if :log_lik in keys(nt5_1s)
        lp = Matrix(nt5_1s.log_lik)
    end
	nparams, ndraws, nchains = 
		(size(lp, 1), m5_1s.method.num_samples, m5_1s.n_chains[1])

	lpr = reshape(lp, size(lp, 1), ndraws, nchains)
	size(lpr)
end

# ╔═╡ 2e2f75f3-6655-406d-889b-adeaf0913dee
2psis_loo(lpr).estimates

# ╔═╡ 8f1ed3d4-331f-423f-b306-781878689d4b
psisloo(m5_1s)

# ╔═╡ 1e2fc02a-6a76-4764-be21-32986e3aed21
begin
	compare(models, :waic)
end

# ╔═╡ c91dfe30-5c62-4817-a2a3-326557647505
begin
	compare(models, :psis)
end
...
begin
	ka5_1s = read_samples(m5_1s, :keyedarray)
	CHNS(ka5_1s)
end
...
git-tree-sha1 = "8aff6daf1b54fcb081465fb569db16bb0c4014cb"
